...
def load_google_article():
    df_gArticles = pd.DataFrame(list(db[coll_name].find()))
    df_gArticles.pop('_id')
    return df_gArticles


def afficherArticle_db(title, texte, author, full_articles):
...
def afficherArticle_google(title, media,  full_articles):
    middle_page.markdown(f"""
                ### [{title}]({''.join(full_articles)})
            """)
    middle_page.markdown(f"""       
...
# Partie 4
res_slider = side_part.slider("Choisissez le nombre d'articles à afficher", 1, 40, 1)

article_tab = db.list_collection_names()
article_tab.remove("coinMarket")

coll_name = side_part.selectbox("Choisissez les articles : ", article_tab)
...
for i in range(0, res_slider):
    ...
    if coll_name == "Cryptonaute" :
        author = df_Garticles['author'][i]
        afficherArticle_db(titre[0], text[0], author[0],
                           full_ref
                           )
    elif coll_name == "GoogleNews" :
